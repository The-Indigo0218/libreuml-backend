...
-- Phase 1 / 01 — Per-user storage quota enforcement (5 MB = 5 242 880 bytes).
--
-- Backward-compatible: both columns declare NOT NULL DEFAULT so existing rows receive
-- quota = 5 MB and used = 0 (recomputable from diagram content sizes if needed).
-- New users inserted after this migration also get the DEFAULT values unless the
-- application layer provides explicit values.

ALTER TABLE users
    ADD COLUMN IF NOT EXISTS storage_quota_bytes BIGINT NOT NULL DEFAULT 5242880,
